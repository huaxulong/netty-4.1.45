...
final class PoolThreadCache {
    ...
    PoolThreadCache(PoolArena<byte[]> heapArena, PoolArena<ByteBuffer> directArena,
        ...
        // 条件一般都成立
        if (directArena != null) {
            // 参数1：tinyCacheSize 512
            // 参数2：numTinySubpagePools 32
            // 参数3：SizeClass.Tiny
            // 创建出来一个长度为 32 的MemoryRegionCache 数组，并且数组内每一个元素 类型为 SubpageMemoryRegionCache 类型。
            // SubpageMemoryRegionCache 包含一个 固定长度{512}的队列
            ...
            // pageSize -> 8192
            // 以2为底，8192的对数 是 多少？ 13   这个值 后面逻辑会使用。
    ...
    boolean add(PoolArena<?> area, PoolChunk chunk, ByteBuffer nioBuffer,
        ...
        // 获取出当前 规格大小 的 MemoryRegionCache 对象。
        MemoryRegionCache<?> cache = cache(area, normCapacity, sizeClass);

        if (cache == null) {
            return false;
        }
        // 参数1：chunk，当前 byteBuf 占用内存 归属chunk
    ...
    private abstract static class MemoryRegionCache<T> {
        ...
        public final boolean add(PoolChunk<T> chunk, ByteBuffer nioBuffer, long handle) {
            // 这一步相当于 将 待归还 内存位置 + 内存大小 .. 信息保存到 entry 对象了。
            ...
            if (!queued) {
                ...
                // 因为 队列满了，缓存失败，需要将 entry 归还到 entry对象池
                entry.recycle();
            }
            // 返回是否 缓存成功。
            return queued;
